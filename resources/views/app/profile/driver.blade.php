    ...
    <div class="principal-layout">
        <div class="p-2">
            ...
            <form action="{{ route('profile.update') }}" method="post" enctype="multipart/form-data">
                ...
                <input class="form-control " readonly type="text" id="name" name="name" placeholder="Nombres" value="{{ $user->name }}">
                @error('name')
                ...
                <input class="form-control " readonly type="text" id="last_name" name="last_name" placeholder="Apellidos" value="{{ $user->last_name }}">
                @error('last_name')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="input-group">
                    <select class="form-select perfil-select" id="document_type" name="document_type">
                        <option value="">{{$user->document_type}}</option>
                    </select>
                    <input class="form-control" readonly type="number" id="document" name="document" placeholder="Cédula" value="{{ $user->document }}">
                    @error('document')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <input class="form-control" readonly type="number" id="phone" name="phone" placeholder="Teléfono" value="{{ $user->phone }}">
                @error('phone')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                @enderror
                <div class="container">
                    <button type="submit" disabled id="guardar_perfil" class="save-button">Guardar</button>
                </div>
            </form>
        </div>
    </div>
